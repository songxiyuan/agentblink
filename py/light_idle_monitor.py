...
The monitor starts only after Codex sets the light to solid green. It watches
macOS HIDIdleTime, which normally increases while the keyboard and mouse are
idle. A clear drop in HIDIdleTime means new keyboard or mouse activity happened,
so the monitor sends the ESP32 an "off" command and exits.
"""
...
def main() -> int:
    previous_idle_ms = current_idle_ms()
    deadline = time.monotonic() + MAX_MONITOR_SECONDS
    while time.monotonic() < deadline:
        idle_ms = current_idle_ms()
        if idle_ms is not None:
            if previous_idle_ms is not None and idle_ms + INPUT_RESET_DROP_MS < previous_idle_ms:
                turn_off()
                break
